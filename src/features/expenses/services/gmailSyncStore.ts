...
const KEY = "nomadsafe.gmail.last-sync-at";
...
export async function loadGmailLastSyncAt(): Promise<number | null> {
  try {
    const raw = await SecureStore.getItemAsync(KEY);
    if (!raw) return null;
    const state = JSON.parse(raw) as GmailSyncState;
...
export async function saveGmailLastSyncAt(timestamp: number): Promise<void> {
  try {
